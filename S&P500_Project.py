...
def process_ticker_data(ticker_data, market_cap, ticker_to_company):
    ...
    if not today_data.empty:
        ...
        # Generate investment suggestion based on the predicted vs. actual
        # close price
        signal = 'Buy' if predicted_close > actual_close else 'Sell'

        # Get the last date the data was updated for this ticker in mm/dd/yyyy
        # format
        last_date = today_data['Date'].strftime('%m/%d/%Y')

        # Add the suggestion, predicted price, actual price, market cap, and
        # last update date to the list
        investment_suggestions.append({
            'Ticker': f'{ticker} ({company_name})',
            'Signal': signal,
            'Pred. Price': predicted_close,
            'Act. Price': actual_close,
            'Difference': predicted_close - actual_close,
...
def fetch_historical_data(tickers, start_date):
    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(
            fetch_ticker_data,
            [(ticker, start_date) for ticker in tickers],
        )
...
def main():
    ...
    sp500_tickers_df = pd.read_html(
        'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies'
    )[0]
    ticker_to_company = dict(
        zip(sp500_tickers_df['Symbol'], sp500_tickers_df['Security'])
    )
    ...
    if market_open:
        ...
        time_since_close = (
            datetime.now(pytz.timezone('US/Eastern')) - last_market_close_time
        )
        hours, remainder = divmod(time_since_close.total_seconds(), 3600)
        minutes = remainder // 60
        print(
            f"MARKET IS CLOSED. It was last open {int(hours)} hours and "
            f"{int(minutes)} minutes ago."
        )
    ...
    stock_data_and_caps = fetch_historical_data(
        ticker_to_company.keys(),
        five_years_ago,
    )

    # Process the stock data and make predictions in parallel
    with mp.Pool(mp.cpu_count()) as pool:
        investment_suggestions_lists = pool.starmap(
            process_ticker_data,
            [
                (data, cap, ticker_to_company)
                for data, cap in stock_data_and_caps
            ],
        )
    ...
    investment_suggestions = [
        item
        for sublist in investment_suggestions_lists
        for item in sublist
    ]
    investment_suggestions_df = pd.DataFrame(investment_suggestions)

    # Sort the suggestions alphabetically by Ticker
    investment_suggestions_df = investment_suggestions_df.sort_values(
        by='Ticker',
        ascending=True,
    )

    # Format numbers with commas and display the DataFrame
    investment_suggestions_df[['Pred. Price', 'Act. Price', 'Market Cap']] = (
        investment_suggestions_df[['Pred. Price', 'Act. Price', 'Market Cap']]
        .applymap(lambda x: f"{x:,.2f}" if isinstance(x, (int, float)) else x)
    )

    # Display the investment suggestions (Ticker, Buy/Sell Signal,
    # Predicted/Actual Price, Market Cap, Last Updated)
    # print(
    #     investment_suggestions_df[
    #         ['Ticker', 'Signal', 'Pred. Price', 'Act. Price',
    #          'Market Cap', 'Last Updated']
    #     ].to_string(index=False, col_space=15)
    # )

    # Generate recommendations for the top 10 "Buy" and "Sell" companies
    # based on the price difference
    top_10_buy = investment_suggestions_df[
        investment_suggestions_df['Signal'] == 'Buy'
    ].nlargest(10, 'Difference')
    top_10_sell = investment_suggestions_df[
        investment_suggestions_df['Signal'] == 'Sell'
    ].nsmallest(10, 'Difference')

    print(
        "\nTop 10 Companies to Buy based on Predicted vs Actual Price "
        "Difference:\n"
    )
    print(
        top_10_buy[
    ...
    print(
        "\nTop 10 Companies to Sell based on Predicted vs Actual Price "
        "Difference:\n"
    )
    print(
        top_10_sell[
